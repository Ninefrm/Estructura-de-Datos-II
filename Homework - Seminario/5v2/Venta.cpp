...
void Venta::addClient(std::string Owner){
  ...
  X="1";
  A.addStatus(X);
Write<<A.sName()<<"#"<<A.sID()<<"#"<<A.sPhone()<<"#"<<A.sEmail()<<"#"<<A.sPayment()<<"#"<<A.sRFC()<<"#"<<A.sStatus()<<"#"<<A.sOwner()<<'\n';
CtS++;
}

void Venta::ShowClients(std::string Owner){
  std::string Tmp;
  ...
  std::cout<<"NAME    "<<"CODE  "<<"PHONE NUMBER   "<<"EMAIL              "<<"PAYMENT        "<<"ADDRESS   "<<std::endl;
    while(!Read.eof()){
      ...
      A.addOwner(Tmp);
      if(Read.eof())break;
      if(A.sStatus()=="1" && (Owner.compare(A.sOwner())==0 || Owner == "GodMode")){
        std::cout<<A.sName()<<" "<<A.sID()<<"      "<<A.sPhone()<<"     "<<A.sEmail()<<"    "<<A.sPayment()<<"    "<<A.sRFC()<<std::endl;
      }
    }Read.close();
}

void Venta::Search(std::string Name, std::string Owner){
  std::string Tmp;
  std::ifstream Read("Clientes.txt");
  if(!Read.good())
    std::cout<<"Error";
...
void Venta::Edit(std::string Name, std::string Owner){
  std::string Tmp;
  int W=1;
  int OP;
  ...
  std::ifstream Read("Clientes.txt");
    while(!Read.eof()){
      ...
      A.addOwner(Tmp);
      if(Read.eof())break;
      if(Name == A.sName() && (Owner.compare(A.sOwner())==0 || Owner == "GodMode")){
        while(W){
    ...
    File<<A.sName()<<"#"<<A.sID()<<"#"<<A.sPhone()<<"#"<<A.sEmail()<<"#"<<A.sPayment()<<"#"<<A.sRFC()<<"#"<<A.sStatus()<<"#"<<A.sOwner()<<'\n';
  }Read.close(); File.close();
    remove("Clientes.txt");
    rename("tmp.txt","Clientes.txt");
}

void Venta::Delete(std::string sName, std::string Owner){
  std::string Tmp;
  int W=1;
  int OP;
  std::ofstream File("tmp.txt", std::ios::app);
  ...
  while(!Read.eof()){
      ...
      A.addOwner(Tmp);
      if(Read.eof())break;
      if(sName != A.sName() && (Owner.compare(A.sOwner())!=0)){
        File<<A.sName()<<"#"<<A.sID()<<"#"<<A.sPhone()<<"#"<<A.sEmail()<<"#"<<A.sPayment()<<"#"<<A.sRFC()<<"#"<<A.sStatus()<<"#"<<A.sOwner()<<'\n';
      }
...
bool Venta::HaveClients(std::string& Owner){
  std::string Tmp;
  std::ifstream Read("Clientes.txt");
  if(!Read.good())
    std::cout<<"Error";
  ...
  std::cout<<"NAME    "<<"CODE  "<<"PHONE NUMBER   "<<"EMAIL              "<<"PAYMENT        "<<"ADDRESS   "<<std::endl;
    while(!Read.eof()){
      ...
      A.addOwner(Tmp);
      if(Read.eof())break;
      if(Owner.compare(A.sOwner())==0){
        Read.close();
        std::cout<<"The profile has clients"<<std::endl;
        return true;
      }
    }Read.close();
    std::cout<<"The profile hasnt clients"<<std::endl;
    return false;
}
